...
export class OrdersService {
  ...
  // }
  async placeOrder(order: PlaceOrderDto) {
    // check limit of trades placed in last 1 minute
    await this.throttleOrders(); // ✅ ADD THIS LINE

    const token = this.tokenService.getToken();
    const baseUrl = this.configService.get<string>('NOREN_BASE_URL');
    const MAGIC = this.configService.get<string>('MAGIC_NUMBER') || '00000000'; // added magic nuber
    ...
    /* ---------------- Building remarks with magic number ---------------- */
    let baseRemark = order.remarks ?? '';

    // prevent duplicate magic number
    if (!baseRemark.includes(`_${MAGIC}_`)) {
      baseRemark = baseRemark ? `${baseRemark}_${MAGIC}_` : `_${MAGIC}_`;
    }

    const finalRemarks = baseRemark;
...
`;
  ...
  // throttle order for controling max 10 order per second limit of Noren API. This is a simple implementation and can be improved with a proper token bucket algorithm if needed.
  private async throttleOrders() {
    const now = Date.now();

    // reset window if 1 sec passed
    ...
    if (this.orderCount > 7) {
      this.logger.warn('⚠️ Rate limit hit → delaying orders');
